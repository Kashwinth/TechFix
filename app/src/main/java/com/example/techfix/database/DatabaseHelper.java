...
public class DatabaseHelper extends SQLiteOpenHelper {
    ...
    @Override public void onCreate(SQLiteDatabase db) {
        ...
        db.execSQL("CREATE TABLE appointments (id INTEGER PRIMARY KEY AUTOINCREMENT,user_id INTEGER NOT NULL,branch_id INTEGER NOT NULL,device_category TEXT NOT NULL,device_model TEXT,issue_description TEXT,price REAL NOT NULL DEFAULT 0,status TEXT NOT NULL DEFAULT 'Pending',technician_id INTEGER,assignment_note TEXT,FOREIGN KEY(user_id) REFERENCES users(id) ON DELETE CASCADE,FOREIGN KEY(branch_id) REFERENCES branches(id) ON DELETE CASCADE,FOREIGN KEY(technician_id) REFERENCES technicians(id) ON DELETE SET NULL)");
    ...
    public boolean addAppointment(Appointment a){ContentValues v=new ContentValues();v.put("user_id",a.getUserId());v.put("branch_id",a.getBranchId());v.put("device_category",a.getDeviceCategory());v.put("device_model",a.getDeviceModel());v.put("issue_description",a.getIssueDescription());v.put("status",a.getStatus());v.put("price",a.getPrice());if(a.getTechnicianName()!=null){int technicianId=findTechnicianId(a.getBranchId(),a.getTechnicianName());if(technicianId>0)v.put("technician_id",technicianId);}if(a.getAssignmentNote()!=null)v.put("assignment_note",a.getAssignmentNote());return getWritableDatabase().insert(TABLE_APPOINTMENTS,null,v)!=-1;}
    ...
    private List<Appointment> appointments(String where,String[] args){List<Appointment> out=new ArrayList<>();String q="SELECT a.*,b.location_name,u.name,t.name AS technician_name FROM appointments a JOIN branches b ON a.branch_id=b.id JOIN users u ON a.user_id=u.id LEFT JOIN technicians t ON a.technician_id=t.id"+(where==null?"":" WHERE "+where)+" ORDER BY a.id DESC";try(Cursor c=getReadableDatabase().rawQuery(q,args)){while(c.moveToNext()){Appointment a=new Appointment();a.setId(c.getInt(c.getColumnIndexOrThrow("id")));a.setUserId(c.getInt(c.getColumnIndexOrThrow("user_id")));a.setBranchId(c.getInt(c.getColumnIndexOrThrow("branch_id")));a.setDeviceCategory(c.getString(c.getColumnIndexOrThrow("device_category")));a.setDeviceModel(c.getString(c.getColumnIndexOrThrow("device_model")));a.setIssueDescription(c.getString(c.getColumnIndexOrThrow("issue_description")));a.setPrice(c.getDouble(c.getColumnIndexOrThrow("price")));a.setStatus(c.getString(c.getColumnIndexOrThrow("status")));a.setBranchName(c.getString(c.getColumnIndexOrThrow("location_name")));a.setUserName(c.getString(c.getColumnIndexOrThrow("name")));a.setTechnicianName(c.getString(c.getColumnIndexOrThrow("technician_name")));a.setAssignmentNote(c.getString(c.getColumnIndexOrThrow("assignment_note")));out.add(a);}}return out;}
